...
function evaluate(program) { 
    ...
    while (! is_null(C)) {
        ...
        if (is_literal(command)) {
            ...
            C = pair(logical_compo_to_conditional(command), C);
...
function logical_compo_to_conditional(logical) {
    const operator = logical_symbol(logical);
    const op1 = logical_composition_first_component(logical);
    const op2 = logical_composition_second_component(logical);
    
    return operator === "&&"
           ? make_conditional_expression(op1, op2, op1)
           : make_conditional_expression(op1, make_literal(true), op2);
}
